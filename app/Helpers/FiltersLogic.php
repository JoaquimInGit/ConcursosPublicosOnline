...
//dd($contests);
     ...
     * Envia notificações consoante os ultimos registos criados na tabela de relação entre os contests e os filters
     */
    public static function sendNotifications2()
    {

        //$time = Carbon::createFromFormat('H', )->toDateTimeString();
       // $actualtime = Carbon::now()->hour(8)->minutes(0)->seconds(0)->milliseconds(0);
        $actualtime = Carbon::now()->minutes(0)->seconds(0)->milliseconds(0);
      // ddd($actualtime);
        $firstscapetime = Carbon::now()->hour('7')->minutes(0)->seconds(0)->milliseconds(0);
        $secondScrape = Carbon::now()->subDay()->hour('13')->minutes(0)->seconds(0)->milliseconds(0);
       // ddd($secondScrape);
        //dd($actualtime->eq($firstscapetime));
        ...
        foreach ($entities as $entity) {
            try {
                ...
                foreach ($filtros as $filtro) {
                    ...
                    //se a hora da primeira notificação for maior que a segunda ou seja já passa das 8 horas
                    if($actualtime->eq($firstscapetime)){

                        //corre as 8
                        //vai buscar os contestfilters das 14 da tarde do dia anterior as 8 da manha daquele dia

                        $contests = ContestFilter::where([['filter_id', $filtro->id], ['created_at', '>', $secondScrape]])->get();
                        //  $contests = ContestFilter::where([ ['created_at', '>', $secondScrape]])->get();

                        //  ddd($contests);
                    }else{
                        //corre as 14
                        //contest filter desde das 8 da manhã as 14 da tarde
                        $contests = ContestFilter::where([['filter_id', $filtro->id], ['created_at', '>', $firstscapetime]])->get();
                        //    $contests = ContestFilter::where([['created_at', '>', $firstscapetime]])->get();
                        //  ddd($contests);
                    }
    ...
    public static function applyFilter2()
    {
        $filters = Filter::where('filter_status', '>=', 1)->get()->toArray();
        $filters = json_encode($filters);
        $filters = json_decode($filters);
        ...
        foreach ($filters as $filter) {
            try {
                // dd($filter->filter_name);
                $contests = Contest::where('created_at', '>=', Carbon::now()->minute(0)->seconds(0)->toDateTimeString('minute'));
